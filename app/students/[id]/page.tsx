...
import { User, ArrowLeft, Mail, Star, Building2 } from 'lucide-react'
        ...
        <div className="flex items-center justify-center min-h-[60vh]">
          <Card className="glass-panel border-0 rounded-um-lg text-center">
            <CardContent className="py-12">
              <h3 className="text-2xl font-semibold mb-3 text-white">学生が見つかりません</h3>
      ...
      <main className="relative z-10 max-w-6xl mx-auto py-10 px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-6 mb-10">
          <div className="w-16 h-16 rounded-um-lg bg-white/5 overflow-hidden flex items-center justify-center">
            <User className="h-7 w-7" style={{ color: 'var(--ink-muted-fallback)' }} />
          </div>
          <div className="flex-1">
            <div className="flex items-center gap-4 mb-2">
              <h1 className="text-3xl font-bold text-white">{student.name}</h1>
              {student.rating_avg !== null && student.rating_avg !== undefined && (
                <div className="flex items-center gap-2 px-3 py-1 rounded-full bg-yellow-500/10 border border-yellow-400/40">
                  ...
                  {student.rating_count && student.rating_count > 0 && (
        ...
        {student.description && (
          <Card className="glass-panel border-0 rounded-um-lg mb-10">
            <CardHeader>
              <CardTitle className="text-xl text-white">プロフィール</CardTitle>
            </CardHeader>
            <CardContent>
              <p className="leading-relaxed whitespace-pre-wrap" style={{ color: 'var(--ink-muted-fallback)' }}>{student.description}</p>
            </CardContent>
          </Card>
        )}
