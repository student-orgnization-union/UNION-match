...
export default function OrganizationsProjectsPage() {
  ...
  const fetchProjects = async () => {
    try {
      ...
      // 学生団体向け: target_typeが'organization'または'both'の案件を取得
    ...
    <div className="relative min-h-screen overflow-hidden text-white motion-fade-in" style={{ background: 'var(--bg-0-fallback)' }}>
      ...
      <main className="relative z-10 mx-auto max-w-5xl px-4 py-20 sm:px-6 lg:px-8">
        <section className="mx-auto max-w-3xl text-center">
          ...
          <p className="mt-4 text-lg" style={{ color: 'var(--ink-muted-fallback)' }}>
            ...
            <Card className="glass-panel border-0 rounded-um-lg">
              <CardContent className="flex flex-col gap-6 py-6 md:flex-row md:items-end md:justify-between">
                ...
                {(searchTerm || budgetFilter !== 'all' || sortBy !== 'newest') && (
                  <Button
                    variant="ghost"
                    className="self-start text-sm hover:text-white"
              ...
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <p className="text-sm" style={{ color: 'var(--ink-muted-fallback)' }}>
                  ...
                  {(searchTerm || budgetFilter !== 'all') && (
                    <span className="ml-2 text-xs" style={{ color: 'var(--ink-muted-fallback)' }}>全{projects.length}件中</span>
                  )}
                </p>
                <div className="flex items-center gap-3 text-xs" style={{ color: 'var(--ink-muted-fallback)' }}>
  ...
  return (
    <Card className="glass-panel border-0 rounded-um-lg transition duration-300 hover:-translate-y-1 hover:border-indigo-400/40 motion-fade-in">
      <CardHeader className="space-y-4 pb-0">
        ...
        <CardDescription className="line-clamp-3 text-sm leading-relaxed" style={{ color: 'var(--ink-muted-fallback)' }}>
          {project.description}
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-6 pt-6">
        <div className="space-y-3 text-sm" style={{ color: 'var(--ink-muted-fallback)' }}>
          {project.company && (
  ...
  return (
    <Card className="glass-panel border-0 rounded-um-lg text-center">
      <CardContent className="space-y-6 py-16">
        ...
        {isFiltered ? (
          <Button variant="outline" className="border-white/20 text-white hover:bg-white/10" onClick={onReset}>
            条件をリセットして一覧を更新
          </Button>
        ) : null}
        {total === 0 && (
          <div className="mt-6 flex items-center justify-center gap-2 text-xs" style={{ color: 'var(--ink-muted-fallback)' }}>
  ...
  return (
    <section className="mx-auto max-w-3xl text-center">
      ...
      <p className="mt-4 text-lg leading-relaxed" style={{ color: 'var(--ink-muted-fallback)' }}>
        UNION Match で公開されている学生団体向け案件から、あなたの団体に合うプロジェクトを発見しましょう。詳細ページでは募集背景や期待する成果まで確認できます。
      </p>
    </section>
  )
}
